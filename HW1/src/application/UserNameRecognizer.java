	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// A-Z, a-z, 0-9 -> State 1
				if ((currentChar >= 'A' && currentChar <= 'Z' ) || (currentChar >= 'a' && currentChar <= 'z')) {	// new code removes 0-9 for the first letter 
				// (only alphabetically)
					nextState = 1;
			...
			case 1: 
				...
				// . -> State 2
				else if (currentChar == '.' || currentChar == '-' || currentChar == '_') { //new code now includes subtraction and underscore
					// Check for /
					nextState = 2;
		...
		case 0:
			...
			userNameRecognizerErrorMessage += "A UserName must start with A-Z, a-z.\n"; //new code specifies user-name to start with a letter and not a digit
